...
((Joomla)=>{
  class joomlaBreadcrumb extends HTMLElement {
    ...
    connectedCallback() {
      ...
      // breadcrumbList.classList.add('all-items');
      ...
      const minimizeItemsFun = () =>{
        if(allItems.length>0){
          ...
          const singleList = document.createElement('ol');
          singleList.appendChild(allItems[0]);
          nav.prepend(minimizeWrapper);
          nav.prepend(singleList);
          // for(let i = filterItems.length - 1; i >= 0; i--){
          for(let i = 1; i >= filterItems.length - 1; i++){
            if(nav.offsetWidth < breadcrumbList.parentElement.offsetWidth){
              breadcrumbList.append(filterItems[i]);
