...
const runScss = () => {
  watch.createMonitor(RootPath, (monitor) => {
    monitor.on('created', (file) => {
      if (file.match(/\.scss/)) {
        const templatesSccs = Path.join(RootPath, 'administrator/templates/khonsu/scss/template.scss');
    ...
    monitor.on('changed', (file) => {
      if (file.match(/\.scss/)) {
        let templatesSccs = Path.join(RootPath, 'administrator/templates/khonsu/scss/template.scss');
